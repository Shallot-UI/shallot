...
<Meta title="👩‍🔧 Building Components / Custom Styling" />
...
Shallot makes it easy to create entirely new styling logic without having to constantly re-write markup and state management for standard components. In this guide, we'll explore how to create new styles for existing components.
...
2. A **stateful** component which manages a static component, and adds stateful behaviour to it. This component is responsible for managing the state of the static component, and is not aware of any presentation.

## Static Components

Let's start by looking at how a static component is styled using the `styles` object. As an example, we'll look at the `<StaticButton />` component. This component is responsible for the presentation of a simple button.

The `StaticButton` consists of a Container and a Label. The Container is a Shallot `<Row />` component, and the Label is a Shallot `<Text />` component. The `styles` property looks like this:
...
Notice that many of these styles are references to theme tokens. You can see an example below of what the button looks like when rendered using the default theme, but these values can be overridden by passing a custom theme to the `<ThemeProvider />` component.
